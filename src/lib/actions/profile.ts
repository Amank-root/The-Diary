...
import { authSessionServer, auth } from '@/lib/auth';
...
export async function updateProfileData(
  formData: FormData
): Promise<{ success: boolean } | { error: ProfileError | string }> {
  ...
  try {
    ...
    if (data.username !== session.user.username) {
      ...
      redirect(
        '/auth/sign-in?message=Session%20revoked.%20Please%20sign%20in%20again%20to%20continue.&type=info'
      );
    }
...
async function ensureAutoFollow(userId: string): Promise<void> {
  ...
  try {
    ...
    if (!existingFollow) {
      ...
      console.log(
        `Auto-followed user ${AUTO_FOLLOW_USER_ID} for user ${userId}`
      );
    }
...
export const getProfile = cache(
  async (username?: string): Promise<ProfileDetails | null> => {
    const session = await authSessionServer();
    if (!session) throw new Error('Unauthorized');
    ...
    try {
      ...
      const [profile, pageCount, currentUser] = await Promise.all([
        profileData,
        page,
        self,
      ]);
      const isSelf =
        session.user.username === username || session.user.id === profile?.id;
