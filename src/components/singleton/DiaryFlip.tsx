...
const Page = React.forwardRef<
  HTMLDivElement,
  { number: number; content: string; date?: Date }
    ...
    <div className="diary-flip-container">
      <div className="mb-6">
        ...
        >
          ...
          <PageCover coverImage={diary?.diaryCoverImage || undefined}>
            {diaryTitle}
          </PageCover>
      ...
      {process.env.NODE_ENV === 'development' && (
        <div className="mt-4 text-xs text-gray-500 text-center">
          State: <i>{bookState}</i> | Orientation: <i>{orientation}</i> | Pages:{' '}
          {pages.length}
        </div>
      )}
    </div>
